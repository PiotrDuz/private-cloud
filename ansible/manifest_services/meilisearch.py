...
def build_kustomization(values: Mapping[str, Any], base: Path) -> dict[str, Any]:
    ...
    if set(values) != required or not all(isinstance(values[key], str) and values[key] for key in required):
        raise ValueError(f"Meilisearch manifest values must contain: {', '.join(sorted(required))}")
    return {
        "apiVersion": "kustomize.config.k8s.io/v1beta1",
        "kind": "Kustomization",
        "images": [{"name": "getmeili/meilisearch", **image_transform(values["image"])}],
        "patches": [
            {
                "target": {"version": "v1", "kind": "PersistentVolume", "name": "meilisearch-data"},
                "patch": json_patch(
                    [
            ...
            {
                "target": {"group": "apps", "version": "v1", "kind": "Deployment", "name": "meilisearch"},
                "patch": json_patch(
                    [
                        {
                            "op": "replace",
