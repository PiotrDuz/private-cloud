...
def build_kustomization(values: Mapping[str, Any], base: Path) -> dict[str, Any]:
    ...
    if set(values) != required or not all(
        isinstance(values[key], str) and values[key] for key in required
    ):
        raise ValueError(
            f"Manticore manifest values must contain: {', '.join(sorted(required))}"
        )
    return {
