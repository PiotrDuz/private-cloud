...
17. Setup ARR STACK
    ...
    3. Give each ARR service a dataset under tank/secure/no-backup/k0s/services with a quota and a dedicated 10Ti PV
    4. Create the shared media-library dataset under tank/secure/no-backup/k0s/services/media-library with a quota and a dedicated 10Ti PV
    ...
    28. Create Sonarr, Radarr, Prowlarr, and qBittorrent datasets under tank/secure/no-backup/k0s/services/<service>.
    29. Use the `private-cloud` WebUI username with the Vault-managed qBittorrent password.
    30. Mount the shared media library read-only in Jellyfin.
18. Setup JELLYFIN
...
22. Observation and alerting
    1. Probe every workload for readiness and liveness so unhealthy containers restart or leave service
    2. Collect workload output, sidecar logs, host journal entries, and Kubernetes events into one store
    3. Parse structured severity and normalize warning, error, fatal, panic, and critical levels before evaluation
    4. Preserve unclassified messages as investigation evidence without assigning an alert level
    5. Heartbeat the log pipeline so missing collection alerts during quiet periods
    6. Gather host storage, hardware, and service metrics against trigger thresholds and a capacity dashboard
    7. Alert on storage growth, hardware health, unfixed errors, stale collectors, and overdue maintenance
    8. Evaluate recognized warning, error, and critical logs on a short interval
    9. Deduplicate and suppress repeated alerts while a problem stays open
    10. Email every Warning-or-higher problem, recovery, and recurring reminder through the mail service
    11. Confirm alert delivery at the destination inbox instead of trusting relay acceptance
    12. Treat missing telemetry as a failure state, not as all-clear
23. Setup AMNEZIAWG
    1. Deploy AmneziaWG in the network-access namespace with a configured UDP hostPort.
    2. Use native stdout/stderr logging for collection by Alloy.
24. Merge design provenance and unresolved differences
    1. Preserve the local ARR no-backup target from 0a58395 pending implementation reconciliation.
        - Remote 96d5309 retains backup storage for persistent ARR applications.
        - Local 0a58395 requires a dataset for every ARR service, including the OpenVPN gateway.
    2. Preserve the local DDNS ownership statements from 0a58395 for design reconciliation.
        - Local 0a58395 states that Traefik maintains DDNS for the domain.
        - Local 0a58395 states that Stalwart maintains DDNS for SMTP.
        - Remote 96d5309 uses a dedicated Cloudflare DDNS token and the dns-system workload.
    3. Interpret the local ARR Internet prohibition as prohibiting direct Internet fallback outside OpenVPN.
        - Local 0a58395 states that the ARR stack has no Internet access and loses connectivity on VPN failure.
        - Both branches require external traffic and DNS through OpenVPN.
    4. Retain the explicit remote TCP 443, TCP 25, and AmneziaWG UDP exposure rules.
        - Local 0a58395 describes only Traefik HTTP, Amnezia UDP, and SMTP as visible host ports.
    5. Preserve the intentional Jellyfin metadata change from 39582c7.
        - Remote 96d5309 retains denied Jellyfin egress and disabled online integrations.
        - Local 39582c7 enables direct Internet metadata and image downloads while retaining other integration restrictions.
    6. Provision ARR quality profiles during installation and select them during operator setup.
        - Local 0a58395 leaves quality-profile selection to the operator.
        - Remote 96d5309 adds resolution preferences and managed private-cloud profiles.
